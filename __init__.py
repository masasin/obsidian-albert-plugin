...
class Plugin(PluginInstance, TriggerQueryHandler):
    ...
    def handleTriggerQuery(self, query: Query):
        ...
        query_string_norm = unicodedata.normalize(
            "NFC", query.string.strip()
        ).casefold()
        ...
        if command_item := self._handle_command_query(
            query_string_norm, query.string.strip()
        ):
    ...
    def _handle_command_query(self, query_norm: str, query_raw: str) -> Optional[Item]:
        ...
        for command in self.config.commands:
            ...
            if query_norm == cmd_name_norm or query_norm.startswith(
                f"{cmd_name_norm} "
            ):
    ...
    def _handle_search_query(self, query_norm: str, vault_path: Path) -> List[Item]:
        ...
        try:
            fzf_command = [
                "fzf",
                "--filter",
                query_norm.encode("utf-8"),
                "--walker-root",
                str(vault_path),
            ]

            p_fzf = subprocess.Popen(fzf_command, stdout=subprocess.PIPE)

            stdout_bytes, _ = p_fzf.communicate()

            if p_fzf.returncode > 1:
                raise subprocess.CalledProcessError(
                    p_fzf.returncode, fzf_command, stdout_bytes
                )
            ...
            return [
                self._create_error_item(
                    "'fzf' Not Found", "Please install it to use this plugin."
                )
            ]
    ...
    def _create_command_item(
        self, command: Command, argument: Optional[str] = None
    ) -> Item:
        ...
        return StandardItem(
            ...
            actions=[
                Action(
                    "config",
                    "Open Config File",
                    lambda p=self.config.config_path: openFile(str(p)),
                )
            ],
        )
